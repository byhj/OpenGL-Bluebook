...
layout (binding = 1) uniform sampler2D tex_grass_length;      
layout (binding = 2) uniform sampler2D tex_grass_orientation; 
layout (binding = 3) uniform sampler2D tex_grass_color;  
layout (binding = 4) uniform sampler2D tex_grass_bend;       
...
void main(void)
{
   vec4 offset = vec4(float(gl_InstanceID >> 10) - 512.0f, 0.0f,
                      float(gl_InstanceID & 0X3FF) - 512.0f, 0.0f);
    ...
    float bend_factor = texture(tex_grass_bend, texcoord).r * 2.0;        
    float bend_amount = cos(Position.y);   
	
	float angle = texture(tex_grass_orientation, texcoord).r * 2.0 * 3.141592;                    
    mat4 rot = construct_rotation_matrix(angle);                                                
    vec4 position = (rot * (Position + vec4(0.0, 0.0, bend_amount * bend_factor, 0.0))) + offset;
                                                                                            
    position *= vec4(1.0, texture(tex_grass_length, texcoord).r * 0.9 + 0.3, 1.0, 1.0);                   
                                                                                                                                    
     // vColor = vec4(random_vector(gl_InstanceID).xyz * vec3(0.1, 0.5, 0.1) + vec3(0.1, 0.4, 0.1), 1.0); 
      //vColor = texture(tex_grass_orientation, texcoord);                                                  
    vColor = texture(grasspalette_texture, texture(tex_grass_color, texcoord).r) 
