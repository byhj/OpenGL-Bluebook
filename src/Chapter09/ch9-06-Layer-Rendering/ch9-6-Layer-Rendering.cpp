...
class Layer: public byhj::Application
{
public:
	Layer():program(0) {}
	...
	void v_Render()
	{
		...
		buffer->proj_matrix = glm::perspective(50.0f, GetAspect(), 0.1f, 1000.0f); // proj_matrix;
		int i;
...
void Layer::init_buffer()
{
	...
	glBindTexture(GL_TEXTURE_2D_ARRAY, array_texture);
	glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_RGBA8, GetScreenWidth(), GetScreenHeight(), 16);

	glGenTextures(1, &array_depth);
	glBindTexture(GL_TEXTURE_2D_ARRAY, array_depth);
	glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_DEPTH_COMPONENT32, GetScreenWidth(), GetScreenHeight(), 16);
