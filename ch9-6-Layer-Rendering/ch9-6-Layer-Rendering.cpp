...
class Layer: public sb6::Application
{
public:
	Layer():program(0) {}
	virtual void init()
	...
	virtual void render()
	{
		...
		buffer->proj_matrix = glm::perspective(50.0f, 1.0f, 0.1f, 1000.0f); // proj_matrix;
		int i;

		for (i = 0; i < 16; i++)
		{
			float fi = (float)(i + 12) / 16.0f;
			buffer->mv_matrix[i] = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -4.0f)) *
...
void Layer::init_shader()
{
	...
	ShowShader.link();
	program_show = ShowShader.program;
	...
	LayerShader.link();
    program_layer = LayerShader.program;
}
